# Finding Best Flight ( Dijkstra )



Using Dijkstra's algorithm to find the best combination of flights to get between two cities, where flights `x`
is better than flights `y` if `x` has lower cost *or* if they are tied in cost, `x` has shorter total flight time.

Concretely, to get from Broome to Fitroy Crossing, flights [530, 112] are better than flights [526, 622]
because, since they both cost $110, the first flights are shorter - 5 hours and 52 minutes compared to 6 hours and 23 minutes. 


```python
import networkx as nx
#A heap can be thought of as a priority queue
#for finding the shortest distance between nodes in a graph, 
...
```python
#Graph: create adjacent dictionary of the graph according to list of flights 
#Node: cities, 
#Edge: flight number

def flight_graphs(all_flights):
    G = {}
    for i in range(len(all_flights)):
        departure = all_flights[i][1] 
        arrival = all_flights[i][2]
        if departure not in G:
            G[departure] = {}
        #get flight information
        G[departure][arrival] = G[departure].get(arrival,[]) + [all_flights[i]]
    return G
...
```python
def find_best_flights(all_flights, origin, destination):
    G = flight_graphs(all_flights)
    path_so_far_heap = []
    #cost, duration, arrival, flight number, and path of origin to origin
    #shortest distance by using heap 
    Heap.heappush(path_so_far_heap, (0, 0, 0, [], [origin]))
    while path_so_far_heap:
        #find the minimum path (Dijkstra)
        cost,duration,arrival,flights,path = Heap.heappop(path_so_far_heap)
        #the last destination in this path
        w = path[-1]
        if w == destination:
            return flights
        else:
            #other neighbors cities
            for x in G[w]:
                for flight in G[w][x]:
                    nFlight, _, _, nDeparture, nArrival, nCost = flight
                    #arrival before departure
                    if arrival < str_to_time(nDeparture):
                        newCost = nCost + cost
                        #if x = origin, there is no layover
                        if duration == 0:
                            newDuration = str_to_time(nArrival) - str_to_time(nDeparture)
                        else:
                            #including layovers
                            newDuration = duration + str_to_time(nDeparture) - arrival + str_to_time(nArrival) - str_to_time(nDeparture)
                        newArrival = str_to_time(nArrival)
                        newFlight = flights + [nFlight]
                        newPath = path + [x]
                        Heap.heappush(path_so_far_heap, (newCost, newDuration, newArrival, newFlight, newPath))
    return None 
```
